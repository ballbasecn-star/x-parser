...
if [[ ! -f "${BUNDLE_DIR}/images/x-parser.tar" ]]; then
...
"${SCP_CMD[@]}" "${BUNDLE_DIR}/images/x-parser.tar" "${DEPLOY_USER}@${DEPLOY_HOST}:${DEPLOY_BASE_DIR}/images/x-parser.tar"

echo "==> 在服务器加载镜像并启动服务"
"${SSH_CMD[@]}" "docker load -i '${DEPLOY_BASE_DIR}/images/x-parser.tar' && \
cd '${DEPLOY_BASE_DIR}/deploy' && \
X_PARSER_IMAGE='${IMAGE_NAME}' \
X_PARSER_VERSION='${IMAGE_TAG}' \
